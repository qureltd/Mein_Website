...
CREATE TABLE IF NOT EXISTS submissions (
  ...
  email TEXT NOT NULL,
  age INTEGER,
  type TEXT NOT NULL CHECK (type IN ('create','speak','build','represent','feature','school','partner','contact','future_me')),
  title TEXT,
...
-- Submissions: anyone can insert (public forms use anon key)
CREATE POLICY "insert_submissions" ON submissions FOR INSERT
  TO anon, authenticated WITH CHECK (true);

-- Phase 0 fix: restrict SELECT to authenticated only (no public read of youth data)
CREATE POLICY "select_submissions_authenticated" ON submissions FOR SELECT
...
-- Stories: public can only see published stories (published_at IS NOT NULL)
CREATE POLICY "select_stories_public_published" ON stories FOR SELECT
  TO anon USING (published_at IS NOT NULL);

CREATE POLICY "select_stories_authenticated_all" ON stories FOR SELECT
  TO authenticated USING (true);
...
-- Consent requests: admin insert only; no public read (guardian emails protected)
CREATE POLICY "insert_consent_requests" ON consent_requests FOR INSERT
  TO authenticated WITH CHECK (true);

CREATE POLICY "select_consent_requests_authenticated" ON consent_requests FOR SELECT
  TO authenticated USING (true);

CREATE POLICY "update_consent_requests_authenticated" ON consent_requests FOR UPDATE
  TO authenticated USING (true) WITH CHECK (true);
